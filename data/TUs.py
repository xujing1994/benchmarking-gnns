...
class TUsDataset(torch.utils.data.Dataset):
    def __init__(self, name, args):
        t0 = time.time()
        self.name = name
        self.raw_dir = args.datadir
        self.processed_dir = args.processed_datadir
        #dataset = TUDataset(self.name, hidden_size=1)
        dataset = LegacyTUDataset(self.name, hidden_size=1, raw_dir=self.raw_dir) # dgl 4.0
    ...
    def get_all_split_idx(self, dataset):
        ...
        """
        root_idx_dir = self.processed_dir
    ...
    def _add_self_loops(self):
        ...
        for split_num in range(10):
            self.train[split_num].graph_lists = [self.self_loop(g) for g in self.train[split_num].graph_lists]
            self.val[split_num].graph_lists = [self.self_loop(g) for g in self.val[split_num].graph_lists]
            self.test[split_num].graph_lists = [self.self_loop(g) for g in self.test[split_num].graph_lists]
